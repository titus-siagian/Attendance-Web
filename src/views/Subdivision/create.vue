<template>
  <v-container>
    <v-form ref="form" @submit.prevent="onCreate">
      <v-card>
        <v-card-title>Tambah Sub Divisi</v-card-title>
        <v-card-text class="space-y-4">
          ...
          <v-text-field
            ...
            counter="3"
            maxLength="3"
            :rules="[(v) => !!v || 'Mohon masukkan id Subdivisi']"
            v-model="data.id"
          ></v-text-field>
          <v-text-field
            filled
            label="Nama Subdivisi"
            placeholder="Subdivisi"
            :rules="[(v) => !!v || 'Mohon masukkan nama Subdivisi']"
            v-model="data.name"
          ></v-text-field>
        </v-card-text>
        <v-card-actions>
...
@Component
export default class CreateSubdivisionPage extends Vue {
  ...
  mounted(): void {
    this.updateTitle('Tambah Subdivisi');
    this.getCompany();
  }

  async onChangeCompany(data: string): Promise<void> {
    try {
      this.data.divisionId = null;
      const {
        data: { data: response },
  ...
  async onCreate(): Promise<void> {
    if (this.form.validate()) {
      ...
      try {
        const {
          data: { data: response },
        } = await this.userService.createSubdivision(this.data);
        this.$toast.success(`Berhasil menambahkan ${response.name}`);
        this.$router.push({ name: 'Subdivision' });
      } catch (error) {

      ...
      <v-card>
        ...
        <v-card-text class="space-y-4">
          ...
          <v-text-field
            ...
            counter="3"
            maxLength="3"
            :rules="[(v) => !!v || 'Mohon masukkan id Sub Divisi']"
            v-model="data.name"
          ></v-text-field>
          <v-text-field
            filled
            ...
            :rules="[(v) => !!v || 'Mohon masukkan nama Sub Divisi']"
            v-model="data.name"
          ></v-text-field>
        </v-card-text>
        <v-card-actions>
...
@Component
export default class UpdateSubdivisionPage extends Vue {
  ...
  mounted(): void {
    this.updateTitle('Update Sub Divisi');
  ...
  async onUpdate(): Promise<void> {
    if (this.form.validate()) {
      ...
      try {
        const {
          data: { data: response },
        } = await this.userService.updateSubdivision(this.data);

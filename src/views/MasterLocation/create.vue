<template>
  <v-container>
    <v-form ref="form" @submit.prevent="onCreate">
      <v-card>
        <v-card-title>Tambah Lokasi</v-card-title>
...
import { CircleMarker, Icon, LatLng, Map, Marker, TileLayer } from 'leaflet';
...
@Component
export default class CreateMasterLocationPage extends Vue {
  ...
  mounted(): void {
    ...
    this.updateTitle('Tambah Lokasi');
  }
  ...
  async onCreate(): Promise<void> {
    if (this.form.validate()) {
      ...
      try {
        ...
        const {
          data: { data: response },
        } = await this.service.add(this.data);
